...
describe("TesouroDireto", function () {
	...
	async function deployFixture() {
		...
		const tesouroDiretoContract = await tesouroDireto.deploy(
			...
			await tesouroNacional.getAddress()
		);
	...
	it("Should create pool and add liquidity", async function () {
		...
		await realTokenizadoSTNContract.connect(tesouroNacional).enableAccount(await tesouroNacional.getAddress());
		...
		await tesouroDiretoContract.connect(tesouroNacional).createLiquidtyPool(1, cvalue(100), cvalue(0.01));

		await realTokenizadoSTNContract.connect(tesouroNacional).enableAccount(await tesouroDiretoContract.getAddress());
		await realTokenizadoSTNContract.connect(tesouroNacional).approve(await tesouroDiretoContract.getAddress(), cvalue(1000000));
		await tpftContract.connect(tesouroNacional).setApprovalForAll(await tesouroDiretoContract.getAddress(), true);
		await tesouroDiretoContract.connect(tesouroNacional).addLiquidtyPool(1, cvalue(100));

		expect(await realTokenizadoSTNContract.connect(tesouroNacional).balanceOf(tesouroNacional.address)).to.equal(cvalue(990000));
		expect(await realTokenizadoSTNContract.connect(tesouroNacional).balanceOf(await tesouroDiretoContract.getAddress())).to.equal(cvalue(10000));
    ...
    it ("Should swap", async function() {
		...
		await realTokenizadoSTNContract.connect(tesouroNacional).enableAccount(await tesouroNacional.getAddress());
		await realTokenizadoSTNContract.connect(tesouroNacional).mint(await tesouroNacional.getAddress(), cvalue(10000000));

		await tesouroDiretoContract.grantRole(await tesouroDiretoContract.CREATOR_LIQUIIDITY_POOL_ROLE(), tesouroNacional.address);
		await tesouroDiretoContract.connect(tesouroNacional).createLiquidtyPool(1, cvalue(100), cvalue(0.01));

		await realTokenizadoSTNContract.connect(tesouroNacional).enableAccount(await tesouroDiretoContract.getAddress());
		await realTokenizadoSTNContract.connect(tesouroNacional).approve(await tesouroDiretoContract.getAddress(), cvalue(10000000));
		await tpftContract.connect(tesouroNacional).setApprovalForAll(await tesouroDiretoContract.getAddress(), true);
		await tesouroDiretoContract.connect(tesouroNacional).addLiquidtyPool(1, cvalue(100000));

        await realTokenizadoSTNContract.connect(tesouroNacional).enableAccount(alice.address);
        await realTokenizadoSTNContract.connect(tesouroNacional).mint(alice.address, cvalue(1000));

        await realTokenizadoSTNContract.connect(tesouroNacional).approve(await tesouroDiretoContract.getAddress(), cvalue(1000));
        await tpftContract.connect(tesouroNacional).setApprovalForAll(await tesouroDiretoContract.getAddress(), true);
        await realTokenizadoSTNContract.connect(alice).approve(await tesouroDiretoContract.getAddress(), cvalue(1000));
        await tesouroDiretoContract.connect(alice).swap(1,cvalue(1000),false,cvalue(9));
